...
void build_tree(int l,int r,int root)
{
	...
	else
	{
		...
		//segment_tree[root]._max=max(segment_tree[root<<1]._max,segment_tree[(root<<2)+1]._max);
		//segment_tree[root]._min=min(segment_tree[root<<1]._min,segment_tree[(root<<2)+1]._min);
